  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-4 gap-12">
        {/* Logo / Brand */}
        <motion.div variants={sectionVariant} className="md:col-span-2 lg:col-span-1">
          <Link to="/" className="text-3xl font-extrabold mb-4 inline-block">
            <span
              style={{ background: `linear-gradient(to right, ${HeroPrimaryColor}, ${HeroAccentColor})` }}
          ...
          <ul className="space-y-2 text-gray-400">
            {navLinks.map((link, i) => (
              <motion.li key={i} whileHover={{ x: 5 }}>
                <Link
                  to={link.path}
                  className="relative hover:text-[#854FEE] transition-colors duration-300 before:content-[''] before:absolute before:w-1 before:h-1 before:rounded-full before:bg-[#854FEE] before:-left-3 before:top-1/2 before:-translate-y-1/2 before:opacity-0 hover:before:opacity-100"
                >
                  {link.name}
                </Link>
              </motion.li>
      ...
      <div className="max-w-6xl mx-auto mt-12 border-t border-gray-800 pt-6 text-center">
        <p className="text-sm text-gray-500">
          Designed with <span style={{ color: HeroAccentColor }}>❤️</span> by Ahmed Nakib
        </p>
      </div>
    </motion.footer>
  );
