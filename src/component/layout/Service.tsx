    ...
    <section id="services" className="bg-[#0B0E14] py-24">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="text-center max-w-3xl mx-auto">
          ...
          >
            My <span className={`bg-clip-text text-transparent bg-gradient-to-r from-[${HeroPrimaryColor}] to-[${HeroAccentColor}]`}>Service Offers</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0 }}
        ...
        >
          {/* Note: You must ensure ServiceData is iterable and available in the context of this component */}
          {ServiceData.map((service) => (
            <motion.div
              key={service.id}
              variants={itemVariants}
              whileHover={{ scale: 1.05, boxShadow: `0 0 40px rgba(133, 79, 238, 0.4)` }} // Custom glow on hover
            ...
            >
              {/* Decorative Gradient Border/Glow on Card */}
              <div
              ...
              <div className="relative z-10">
                <div className={`mx-auto h-16 w-16 p-3 rounded-full flex items-center justify-center mb-6 bg-gradient-to-br from-[${HeroPrimaryColor}] to-[${HeroMidColor}]/80 shadow-lg`}>
                  {/* Service Image/Icon */}
                  <img 
                    src={service.image} 
                    alt={service.name} 
                    className="h-10 w-10 object-contain filter brightness-125" // Adjust brightness for dark background
                  />
                </div>
